    ...
    private function getQueryFilter($criteria) {
        ...
        foreach ($criteria as $key => $param) {
            switch ($param) {
                case 'name':
                    if ($count == 0) {
                        $query['where'] = "(com_a.name=com_b.name) ";
                        $query['group'] = "com_a.name";
                    } else {
                        $query['where'] .= " AND (com_a.name=com_b.name) ";
                        $query['group'] .= ",com_a.name";
                    }
                    break;
                ...
                case 'address':
                    if ($count == 0) {
                        $query['where'] = "(address_a.address=address_b.address) ";
                        $query['group'] = "address_a.address";
                    } else {
                        $query['where'] .= " AND (address_a.address=address_b.address)";
                        $query['group'] .= ",address_a.address";
                    }
                    break;
                case 'city':
                    if ($count == 0) {
                        $query['where'] = "(address_a.city=address_b.city) ";
                        $query['group'] = "address_a.city";
                    } else {
                        $query['where'] .= " AND (address_a.city=address_b.city)  ";
                        $query['group'] .= ",address_a.city";
                    }
                    break;
                ...
                case 'state':
                    if ($count == 0) {
                        $query['where'] = "(address_a.state=address_b.state) ";
                        $query['group'] = "address_a.state";
                    } else {
                        $query['where'] .= " AND (address_a.state=address_b.state)";
                        $query['group'] .= ",address_a.state";
                    }
                    break;
                ...
                case 'zip_code':
                    if ($count == 0) {
                        $query['where'] = "(address_a.zip_code=address_b.zip_code) ";
                        $query['group'] = "address_a.zip_code";
                    } else {
                        $query['where'] .= " AND (address_a.zip_code=address_b.zip_code) ";
                        $query['group'] .= ",address_a.zip_code";
                    }
                    break;
                ...
                case 'phone':
                    if ($count == 0) {
                        $query['where'] = "(phone_a.phone=phone_b.phone) ";
                        $query['group'] = "phone_a.phone";
                    } else {
                        $query['where'] .= "AND (phone_a.phone=phone_b.phone) ";
                        $query['group'] .= ",phone_a.phone";
                    }
                    break;
                ...
                case 'first_name':
                    if ($count == 0) {
                        $query['where'] = "(con_a.first_name=con_b.first_name) ";
                        $query['group'] = "con_a.first_name";
                    } else {
                        $query['where'] .= "AND (con_a.first_name=con_b.first_name)";
                        $query['group'] .= ",con_a.first_name";
                    }
                    break;
                ...
                case 'last_name':
                    if ($count == 0) {
                        $query['where'] = "(con_a.last_name=con_b.last_name) ";
                        $query['group'] = "con_a.last_name";
                    } else {
                        $query['where'] .= "AND (con_a.last_name=con_b.last_name)";
                        $query['group'] .= ",con_a.last_name";
                    }
                    break;
                ...
                case 'email':
                    if ($count == 0) {
                        $query['where'] = "(con_a.email=con_b.email) ";
                        $query['group'] = "con_a.email";
                    } else {
                        $query['where'] .= "AND (con_a.email=con_b.email)";
                        $query['group'] .= ",con_a.email";
                    }
                    break;
                ...
                case 'company':
                    if ($count == 0) {
                        $query['where'] = "(com.company_id=com_b.company_id) ";
                        $query['group'] = "com.company_id";
                    } else {
                        $query['where'] .= "AND (com.company_id=com_b.company_id)";
                        $query['group'] .= ",com.company_id";
                    }
                    break;
            }
        ...
        switch ($type) {
            ...
            case 'company':
                ...
                foreach ($selections as $selection) {
                    ...
                    $selectedCompany = $this->ci->Company_model->get_company($companyId, true);
                    if ((sizeof($selectedCompany) == 0) || (sizeof($parentCompany) == 0)) {
                        return false;
                    }

                    if ($selectedCompany['company_id'] != $parentCompany['company_id']) {
                ...
                case 'state':
                    if (strlen($criteria[$key]) > 0) {
                        if ($criteria[$key] == 'empty') {
                            ...
                            $db->where('addresses.state', $param);
                        }
                    }
                    break;
                ...
                case 'zip_code':
                    if (strlen($criteria[$key]) > 0) {
                        if ($criteria[$key] == 'empty') {
                            ...
                            $db->where('addresses.zip-code', $param);
                        }
                    }
                    break;
                ...
                case 'street_address':
                    if (strlen($criteria[$key]) > 0) {
                        if ($criteria[$key] == 'empty') {
                            ...
                            $db->where('addresses.address', $param);
                        }
                    }
                    break;
