...
class Property extends CI_Controller
{
	...
	function ajaxSearch($loadProperties = true)
	{
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			$searchCriteria = array(
				...
				'owner_website' => $this->input->post('owner_website', true),
				'last_sold_date' => $this->input->post('last_sold_date', true),
			);
	...
	function add_history()
	{
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			$propertyID = $this->input->post('property_id', true);
			$this->form_validation->set_rules("last_sold_date", "Last Sold Date", 'trim|required');
			$this->form_validation->set_rules("last_sold_price", "Last Sold Price", 'trim|required|numeric');

			if (($this->form_validation->run())) {
				$lastSoldDate = $this->input->post('last_sold_date', true);
				$propertyHistoryData = array(
					'last_sold_date' => ($lastSoldDate != null) ? date('Y-m-d', strtotime($lastSoldDate)) : null,
					'last_sold_price' => $this->input->post('last_sold_price', true),
					'property_id' => $propertyID
				);

				$this->Property_model->save_new_history($propertyHistoryData);
				$this->session->set_flashdata("success", "property history added successfully");
				redirect(base_url("property/view/$propertyID"));
			}else{
				$this->session->set_flashdata("error", "please enter valid value");
				redirect(base_url("property/view/$propertyID"));
			}
		}
			...
			if ($this->input->server('REQUEST_METHOD') == 'POST') {
				...
				if (($this->form_validation->run())) {
					...
					$propertyData = array(
						...
						'last_update' => ($lastUpdateDate != null) ? date('Y-m-d', strtotime($lastUpdateDate)) : null,
						'availability_update_date' => ($availabilityUpdateDate != null) ? date('Y-m-d', strtotime($availabilityUpdateDate)) : null,
						'availability_status' => (($availabilityStatus != null) && (sizeof($availabilityStatus) > 0)) ? implode(",", $availabilityStatus) : "",
						'property_link' => $this->input->post('property_link', true),
						'lease_type' => $this->input->post('lease_type', true),
						'annual_rent' => $this->input->post('annual_rent', true),
						'asking_cap_rate' => $this->input->post('asking_cap_rate', true),
	...
	function view($propertyId)
	{
		...
		if (isset($data['property']['property_id'])) {
			...
			$propertyHistory = $this->Property_model->get_histories($propertyId);
			...
			$data['propertyHistories'] = $propertyHistory;
	...
	function remove_history($property_history_id)
	{
		$propertyHistory = $this->Property_model->get_history($property_history_id);
		if (isset($propertyHistory['property_history_id'])) {
			$this->Property_model->delete_history($property_history_id);
			$this->session->set_flashdata("success", "property deleted");
			redirect(base_url("property/view/" . $propertyHistory['property_id']));
		} else {
			redirect(base_url());
		}
	}
