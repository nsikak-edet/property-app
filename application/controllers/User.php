...
class User extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->library("bcrypt");
              ...
              //echo $this->bcrypt->hashPassword('password123'); exit;
    }
            ...
            if ($user != null) {
                ...
                if ($this->input->server('REQUEST_METHOD') == 'POST') {
                    ...
                    if (isset($_POST['new_password']) && strlen($_POST['new_password']) > 0) {
                        ...
                        $this->form_validation->set_rules('confirm_new_password', "Confirm New Password", 'trim|required|matches[password]');
                    }
                    $this->form_validation->set_error_delimiters('', '');

                    if ($this->form_validation->run() == true) {
                        $passwordFromUser = $this->input->post('password', true);
                        $hashedPassword = $this->bcrypt->hashPassword($passwordFromUser);
                        $newRecord = array(
                            'first_name' => $this->input->post('first_name', true),
                            'last_name' => $this->input->post('last_name', true),
